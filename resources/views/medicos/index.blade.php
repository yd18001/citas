...
@section('title', 'Gestionar | Medicos')
@section('content')
        ...
        <table class="table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{App\Models\Especialidad::find($medico->id_especialidad)->nombreEspecialidad}}</td>
                    <td>
                        <form action="{{ route ('medicos.destroy',$medico->id)}}" method="POST">
                            <a href="/medicos/{{$medico->id}}/edit" class="btn btn-info" title="Edita medico">
                            ...
                            @csrf
                            @method('DELETE')
                            <button type="submit"class="btn btn-danger" title="Elimina Medico" onclick="return confirm('¿Está seguro de que desea eliminar al médico?')">
